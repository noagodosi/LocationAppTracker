...
import { View, Text, TouchableOpacity, StyleSheet, Switch, PermissionsAndroid,Platform , SafeAreaView, ScrollView, NativeModules} from 'react-native';
import { check, request } from '@react-native-community/permissions';
import RNFS from 'react-native-fs';
import GetLocation from 'react-native-get-location';

const LocationSampling = () => {
     ...
     useEffect(() => {
         const fetchDataFromFile = async () => {
           try {
             ...
             console.log('Location data loaded:', parsedData);
           } catch (error) {
...
// Request permissions to get the device location
    useEffect(() => {
        ...
        try {
            ...
            if (locationGranted === PermissionsAndroid.RESULTS.GRANTED && storageGranted === PermissionsAndroid.RESULTS.GRANTED) {
...
// Sample user location each 5 second
  useEffect(() => {
    const intervalId = setInterval(getLocationData, 5000); // Fetch data every 5 seconds
    return () => clearInterval(intervalId);
  }, []);

  // Save the new location list to device file
  useEffect(() => {
    ...
    const getLocationData = async () => {
          try {
              ...
              const newSample = {
                location,
                timestamp: {date, time},
                displayState: locked === null ? 'Loading...' : isLocked ? 'Locked' : 'Unlocked',
          };

			setLocationData((prev) => [newSample, ...prev])
          } catch (error) {
    ...
    <ScrollView>
     <Text style={styles.subtitle}>Please select mode</Text>
               <View style={styles.switchContainer}>
                 <Text style={styles.text}>{isSampling ? "Stop Sampling" : "Start Sampling"}</Text>
                <Switch
                                trackColor={{ false: "gray", true: "#81b0ff" }}
                                thumbColor={isSampling ? "#EEEDEB" : "#f4f3f4"}
                                onValueChange={toggleSampling}
                                value={isSampling}
                                  style={{ transform: [{ scaleX: 1.3 }, { scaleY: 1.3 }] , marginTop:20, marginHorizontal:10}}
                              />
               </View>

      <Text style={styles.textLocation}>Current Location: {currentLocation ? `${currentLocation.latitude}, ${currentLocation.longitude}` : 'Unknown'}</Text>
   </ScrollView>
   </SafeAreaView>
  );
};
